...
public class DependenciesUnitTestCase {
    ...
    /**
     * Tests that artifact is cannot be packaged, but is is resolved right. This test is not executed but shows that some jars
     * cannot be packaged
     */
    // @Test(expected =
    ...
    public void testSimpleResolution() throws ResolutionException {
        ...
        WebArchive war = ShrinkWrap.create(WebArchive.class, name + ".war").addAsLibraries(
                DependencyResolvers.use(MavenDependencyResolver.class).artifact("org.jboss.shrinkwrap.test:test-deps-c:1.0.0")
